...
namespace IO_laba_2
{
    ...
    public class SimplexData
    {
        ...
        public SimplexData(SimplexData CloneData)
        {
            num_of_vars_in_f = CloneData.num_of_vars_in_f;
            num_of_vars = CloneData.num_of_vars;
            num_of_constraints = CloneData.num_of_constraints;
            direction = CloneData.direction;
            coefficients = new List<double>(CloneData.coefficients);
            conditions = new List<string>(CloneData.conditions);
            basis = new List<Optional<int>>(CloneData.basis);
            artificial_variables = new HashSet<int>();
            deltas = new List<double>();
            constraints_coefficients = new List<List<double>>();
            foreach (List<double> line in CloneData.constraints_coefficients)
            {
                List<double> CloneList = new List<double>(line);
                constraints_coefficients.Add(CloneList);
            }
        }
        ...
        public SimplexResult get_result()
        {
            List<double> result = new List<double>(new double[num_of_vars_in_f]);
            for (int i = 0; i < basis.Count; i++)
            {
                if (basis[i].HasValue && basis[i].Value < num_of_vars_in_f)
                {
                    result[basis[i].Value] = constraints_coefficients[i][num_of_vars];
                }
            }
        ...
        public void add_condition(int x_index, string condition, double value)     
        {
            List<double> vec = new List<double>(new double[num_of_vars]);
            vec.Add(value);
            vec[x_index] = 1.0;

            conditions.Add(condition);
            constraints_coefficients.Add(vec);
            num_of_constraints += 1;
            basis.Add(new Optional<int>());
        }

        public void add_constraint(List<double> vec, string condition, double value)
        {
            coefficients.Add(0);
            foreach (List<double> con in constraints_coefficients)
            {
                con.Add(con[num_of_vars]);
                con[num_of_vars] = 0.0;
            }
            num_of_vars += 1;
            vec.Add(value);           
