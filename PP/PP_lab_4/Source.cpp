...
size_t colorize(const graph_t& graph, const vector<size_t>& order) {
    ...
    for (size_t v : order) {
        ...
        colored[v] = 1;               // помечаем вершину как раскрашенную
...
size_t MPIColorize(const graph_t& graph, int world_rank, int world_size) {
    size_t size = graph.size();
    ...
    for (size_t i = 0; i < chunk_size; ++i) {
        ...
        local_min = min(local_min, colorize(graph, order));
    }
...
void broadcast_graph(graph_t& graph, int world_rank) {
...
int main(int argc, char* argv[]) {
    ...
    for (size_t i = 0; i < cnt; ++i) {
        ...
        broadcast_graph(graph, world_rank);     // Процесс 0 передает граф всем остальным процессам

        auto start = system_clock::now();
        size_t res = MPIColorize(graph, world_rank, world_size);
